...
## Физико-информированный режим

### Зачем
Физическая регуляризация позволяет привязать прогнозы WLPR к простейшей динамике баланса между добывающими и нагнетательными скважинами. Модель учится не только минимизировать статистический разрыв `y − y_hat`, но и выполняет приближенное условие:

```
ΔWLPR ≈ α * Q_inj − β * WLPR_prev
```

где "α" (`physics_injection_coeff`) масштабирует вклад суммарной закачки по выбранным признакам, а "β" (`physics_damping`) отвечает за естественное затухание дебита. Такая форма отражает, что рост закачки должен тянуть дебит вверх, а без подпитки система склонна возвращаться к базовому уровню.

### Математическая форма штрафа
Внутри `PhysicsInformedLoss` конструируется разность

```
residual_t = (WLPR_t − WLPR_{t−1}) − (α * inj_t − β * WLPR_{t−1})
```

и суммарное наказание вычисляется как среднее `residual_t^2` по горизонту. Если включен `physics_smoothing_weight`, дополнительно штрафуется резкая смена `residual_t`. Итоговая функция потерь:

```
L = L_data + physics_weight * L_physics
```

где `L_data` — выбранная базовая точечная метрика (по умолчанию Huber).

### Требования к данным
- В `config.futr_exog` должны присутствовать признаки, описывающие действие нагнетателей (по умолчанию `inj_wwir_weighted`).
- Для корректного штрафа необходимы непрерывные временные ряды WLPR и закачки без больших пропусков. Предварительная импутация и сглаживание реализованы в пайплайне, но сильные пробелы могут ослабить физический эффект.

### Параметры `PipelineConfig`
- `loss="physics"` — включает физико-информированный режим.
- `physics_base_loss` — базовая статистическая часть (`huber`, `mae`, `mse`, ...).
- `physics_weight` — относительный вес физического штрафа. Стартовое значение 0.1–0.3; повышайте при излишне "резвых" прогнозах.
- `physics_injection_coeff` (α) — масштаб влияния закачки. Подбирайте по порядку величины WLPR/признаков закачки.
- `physics_damping` (β) — насколько быстро дебит стремится к базовому уровню при отсутствии подпитки. Рост параметра усиливает затухание.
- `physics_smoothing_weight` — сглаживание `residual_t`; полезно при шумных данных, но увеличивает время обучения.
- `physics_features` — список признаков из `futr_exog`, которые интерпретируются как закачка. Можно указать несколько (например, с лагами), штраф будет использовать их среднее.

### Как запустить
1. В `main()` или конфигурационном файле задайте нужные параметры, например:
   ```python
   config = PipelineConfig(
       loss="physics",
       physics_weight=0.2,
       physics_injection_coeff=0.08,
       physics_damping=0.01,
   )
   ```
2. Запустите пайплайн как обычно: `python src/wlpr_pipeline.py --data-path ...`.
3. Результаты появятся в выбранной директории артефактов; в `metadata.json` сохранится полный набор параметров запуска.

### Диагностика и контроль
- Метрики `train_data_loss` и `train_physics_penalty` пишутся в `lightning_logs/*/events.out.tfevents.*`. Откройте их в TensorBoard, чтобы наблюдать баланс между точностью и физическим штрафом.
- Если `train_physics_penalty` остаётся высоким, попробуйте увеличить `physics_weight` или уточнить `physics_injection_coeff`.
- При чрезмерном ухудшении метрологий (MAE/RMSE) уменьшайте `physics_weight` либо ослабляйте затухание (`physics_damping`).

### Ограничения
- Штраф опирается на линейную связь между закачкой и дебитом. Для явно нелинейных режимов потребуется кастомизация.
- Физический сигнал берётся из `futr_exog`. Если признаки неправильно подготовлены или отсутствуют, модель вернётся к чисто статистическому поведению и выведет предупреждение в лог.
- Значение штрафа нормируется на маску валидных точек; при длинных пропусках вклад физики ослабляется.
